...
게이트웨이 현대화 실험을 계속 관리할 수 있도록 테스트와 빌드 기준을 보강했습니다.

- 대상 프레임워크를 `.NET 8` LTS로 변경했습니다.
- `DataParserTests`를 추가해 ASCII 파싱과 센서 타입 매핑을 검증했습니다.
- CI에서 빌드와 테스트를 함께 실행하도록 변경했습니다.
- 저장소에 남아 있는 NuGet 산출물 정리 계획은 [정리 노트](docs/cleanup-plan.md)에 정리했습니다.
